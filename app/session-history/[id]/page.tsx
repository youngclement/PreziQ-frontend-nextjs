...
export default function SessionHistoryPage() {
  ...
  useEffect(() => {
    if (
      selectedParticipant &&
      !participantSubmissions.has(selectedParticipant)
    ) {
  ...
  const fetchSession = async () => {
    try {
      ...
      const foundSession = response.data.content.find(
      ...
      if (foundSession) {
        ...
        toast({
          title: t('error'),
          description: t('sessionHistory.error.sessionNotFound'),
          variant: 'destructive',
        });
  ...
  const fetchParticipants = async () => {
    try {
      ...
      toast({
        title: t('error'),
        description: t('sessionHistory.error.participantsFailed'),
        variant: 'destructive',
      });
    } finally {
  ...
  const fetchParticipantSubmissions = async (participantId: string) => {
    ...
    try {
      ...
      const response = await sessionsApi.getParticipantSubmissions(
        sessionId,
        participantId
      );
      ...
      toast({
        title: t('error'),
        description: t('sessionHistory.error.submissionsFailed'),
        variant: 'destructive',
      });
    } finally {
  ...
  const generateActivityStats = () => {
    ...
    participants.forEach((participant) => {
      const submissions = participantSubmissions.get(
  ...
  const getOverallStatsData = () => {
    ...
    const totalIncorrect = participants.reduce(
      (sum, p) => sum + p.finalIncorrectCount,
      0
    );

    return [
      {
        name: t('sessionHistory.detail.correctAnswers'),
        value: totalCorrect,
        color: COLORS.correct,
      },
      {
        name: t('sessionHistory.detail.incorrectAnswers'),
  ...
  const getParticipantPerformanceData = () => {
    return participants
      ...
      .map((p) => ({
        name:
          p.displayName.length > 10
            ? p.displayName.substring(0, 10) + '...'
            : p.displayName,
        score: p.finalScore,
        correctRate:
          p.finalCorrectCount + p.finalIncorrectCount > 0
            ? Math.round(
                (p.finalCorrectCount /
                  (p.finalCorrectCount + p.finalIncorrectCount)) *
                  100
              )
            : 0,
  ...
  if (loading) {
    return (
      <>
        <SimpleHeader
          showBackButton
          title={t('sessionHistory.detail.sessionOverview')}
        />
  ...
  if (!session) {
    return (
      <>
        <SimpleHeader
          showBackButton
          title={t('sessionHistory.error.sessionNotFound')}
        />
        <div className='pt-[52px] container mx-auto py-8'>
          <div className='text-center py-12'>
            <h2 className='text-2xl font-bold mb-4'>
              {t('sessionHistory.error.sessionNotFound')}
            </h2>
            <p className='text-muted-foreground mb-8'>
              The session details you're looking for could not be found.
            </p>
            <Link href='/session-history'>
              <Button>
                <ArrowLeft className='mr-2 h-4 w-4' />
                {t('sessionHistory.detail.backToHistory')}
              </Button>
            </Link>
          </div>
        </div>
                  ...
                  <div className='flex items-center gap-3 p-3 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800'>
                    ...
                    <div>
                      ...
                      <div className='text-sm text-muted-foreground'>
                        {t('sessionHistory.detail.duration')}
                      </div>
                    </div>
                  </div>
                  <div className='flex items-center gap-3 p-3 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800'>
                    ...
                    <div>
                      ...
                      <div className='text-sm text-muted-foreground'>
                        {t('sessionHistory.detail.participants')}
                      </div>
                    </div>
                  </div>
                  <div className='flex items-center gap-3 p-3 rounded-lg bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800'>
                ...
                {loadingParticipants ? (
                  ...
                  <div>
                    ...
                    <Tabs
                      ...
                      className='bg-white dark:bg-slate-900 rounded-lg p-1 shadow-sm border border-slate-200 dark:border-slate-800'
                    >
                      <TabsList className='mb-4 grid grid-cols-3 bg-slate-100 dark:bg-slate-800'>
                        ...
                        >
                          ...
                          <span>
                            {t('sessionHistory.detail.tabs.analytics')}
                          </span>
                        </TabsTrigger>
                        <TabsTrigger
                          value='players'
                        ...
                        >
                          ...
                          <span>
                            {t('sessionHistory.detail.tabs.leaderboard')}
                          </span>
                        </TabsTrigger>
                        <TabsTrigger
                          value='questions'
                        ...
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                          ...
                          <Card className='shadow-sm border border-slate-200 dark:border-slate-800'>
                            <CardHeader className='pb-2'>
                              <CardTitle className='text-lg'>
                                {t('sessionHistory.detail.overallStats')}
                              </CardTitle>
                              <CardDescription>
                                Correct vs. Incorrect answers across all
                                participants
                              </CardDescription>
                            </CardHeader>
                              ...
                              <div className='space-y-4'>
                                <div className='flex justify-between items-center'>
                                  <span className='font-medium'>
                                    {t('sessionHistory.detail.totalQuestions')}:
                                  </span>
                          ...
                          <Card className='shadow-sm border border-slate-200 dark:border-slate-800'>
                            <CardHeader className='pb-2'>
                              <CardTitle className='text-lg'>
                                {t(
                                  'sessionHistory.detail.participantPerformance'
                                )}
                              </CardTitle>
                              <CardDescription>
                                Comparing scores and success rates
                              </CardDescription>
                            </CardHeader>
                            <CardContent className='h-64'>
                              <ResponsiveContainer width='100%' height='100%'>
                                <RadarChart
                                  outerRadius={70}
                                  data={getParticipantPerformanceData().slice(
                                    0,
                                    5
                                  )}
                                >
                                  ...
                                  />
                                  <Radar
                                    name={t('sessionHistory.detail.score')}
                                    dataKey='score'
                                    stroke={COLORS.blue}
                                    fill={COLORS.blue}
                                    fillOpacity={0.4}
                                  ...
                                  <Radar
                                    name={`${t(
                                      'sessionHistory.detail.accuracy'
                                    )} (%)`}
                                    dataKey='correctRate'
                                    stroke={COLORS.cyan}
                                    fill={COLORS.cyan}
                          ...
                          <Card className='shadow-sm border border-slate-200 dark:border-slate-800'>
                            <CardHeader className='pb-2'>
                              <CardTitle className='text-lg'>
                                {t('sessionHistory.detail.questionPerformance')}
                              </CardTitle>
                              <CardDescription>
                                Success rates across questions
                              </CardDescription>
                            </CardHeader>
                            <CardContent className='h-64'>
                              <ResponsiveContainer width='100%' height='100%'>
                                    ...
                                    >
                                      <div className='relative mb-4'>
                                        ...
                                        >
                                          ...
                                          />
                                          <AvatarFallback>
                                            {participant.displayName?.charAt(
                                              0
                                            ) || '?'}
                                          </AvatarFallback>
                                        </Avatar>
                                        <div
                                          className={`absolute -top-3 -right-3 h-10 w-10 flex items-center justify-center rounded-full shadow-md 
                                                                                ${
                                                                                  index ===
                                                                                  0
                                      ...
                                      <span className='font-bold text-center mt-2 text-lg'>
                                        {participant.displayName}
                                      </span>
                                      <span className='text-2xl font-black mt-1 mb-3'>
                                        {participant.finalScore} pts
                                      </span>
                      ...
                      <TabsContent value='players' className='mt-4 space-y-6'>
                        ...
                        <Card className='mb-6 shadow-sm border border-slate-200 dark:border-slate-800'>
                          <CardHeader className='pb-2'>
                            ...
                            <CardDescription>
                              Overall participant performance chart
                            </CardDescription>
                          </CardHeader>
                          <CardContent className='h-64'>
                            <ResponsiveContainer width='100%' height='100%'>
                            ...
                            <TableHeader className='bg-slate-50 dark:bg-slate-800'>
                              <TableRow>
                                <TableHead className='w-12'>
                                  {t('sessionHistory.detail.rank')}
                                </TableHead>
                                <TableHead>
                                  {t('sessionHistory.detail.participants')}
                                </TableHead>
                                <TableHead className='text-right'>
                                  {t('sessionHistory.detail.score')}
                                </TableHead>
                                <TableHead className='text-center'>
                                  {t('sessionHistory.detail.correctAnswers')}
                                </TableHead>
                                <TableHead className='text-center'>
                                  {t('sessionHistory.detail.incorrectAnswers')}
                                </TableHead>
                                <TableHead className='text-center'>
                                  {t('sessionHistory.detail.accuracy')}
                                </TableHead>
                                <TableHead></TableHead>
                              </TableRow>
                            </TableHeader>
                        ...
                        <div className='space-y-6'>
                          ...
                          <Card className='shadow-sm border border-slate-200 dark:border-slate-800'>
                            <CardHeader className='pb-2'>
                              ...
                              <CardDescription>
                                Performance statistics for all questions
                              </CardDescription>
                            </CardHeader>
                            <CardContent>
                              <Table>
                                <TableHeader>
                                  <TableRow>
                                    ...
                                    <TableHead className='text-center w-20'>
                                      {t(
                                        'sessionHistory.detail.correctAnswers'
                                      )}
                                    </TableHead>
                                    <TableHead className='text-center w-20'>
                                      {t(
                                        'sessionHistory.detail.incorrectAnswers'
                                      )}
                                    </TableHead>
                                    <TableHead className='text-center w-24'>
                                      {t('sessionHistory.detail.accuracy')}
                                  ...
                                  {activityStats.map((stat, index) => (
                                    <TableRow key={stat.activityId}>
                                      ...
                                      <TableCell className='text-center text-sm'>
                                        {stat.responses[0]?.activity?.activityType?.replace(
                                          '_',
                                          ' '
                                        ) || 'Unknown'}
                                      </TableCell>
                          ...
                          <Card className='shadow-sm border border-slate-200 dark:border-slate-800'>
                            <CardHeader className='pb-2'>
                              ...
                              <CardDescription>
                                Number of correct and incorrect answers per
                                question
                              </CardDescription>
                            </CardHeader>
                            <CardContent className='h-64'>
                      ...
                      <TabsContent value='submissions' className='mt-4'>
                        {selectedParticipant ? (
                              ...
                              (() => {
                                ...
                                return submissions.length > 0 ? (
                                  <Table>
                                    ...
                                    <TableBody>
                                      {submissions.map((submission) => (
                                        <TableRow
                                          key={submission.activitySubmissionId}
                                        >
                                          <TableCell>
                                            <div className='font-medium'>
                                  ...
                                  <div className='text-center py-8 text-muted-foreground'>
                                    {t('sessionHistory.detail.noSubmissions')}
                                  </div>
                                );
                              })()
                            )}
                          ...
                          <div className='text-center py-8 text-muted-foreground'>
                            {t('sessionHistory.detail.selectParticipant')}
                          </div>
                        )}
                      </TabsContent>
                    </Tabs>
                  ...
                  <div className='text-center py-8'>
                    <div className='text-muted-foreground'>
                      {t('sessionHistory.detail.noParticipants')}
                    </div>
                  </div>
                )}
              </CardContent>
