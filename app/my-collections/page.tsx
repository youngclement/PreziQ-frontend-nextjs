...
export default function MyCollectionsPage() {
    ...
    const fetchMyCollections = async (page = 1, size = 12) => {
        ...
        try {
            ...
            setError('Không thể tải danh sách bộ sưu tập của bạn. Vui lòng thử lại sau.');
            toast({
                title: 'Lỗi',
                description: err instanceof Error ? err.message : 'Không thể tải danh sách bộ sưu tập của bạn.',
                variant: 'destructive',
            });
        } finally {
            setIsLoading(false);
    ...
    const handleDeleteCollection = async (id: string) => {
        ...
        try {
            ...
            if (response?.data?.success) {
                setCollections(prev => prev.filter(c => c.id !== id && c.collectionId !== id));
    ...
    const handleTogglePublish = async (collection: Collection, event?: React.MouseEvent) => {
                        ...
                        {pagination.totalPages > 1 && (
                            <div className="flex justify-center mt-8">
                                <div className="flex space-x-4">
                                    <button
                                        onClick={() => fetchMyCollections(pagination.currentPage - 1, pagination.pageSize)}
                                        disabled={!pagination.hasPrevious}
                                        className={`relative flex button-group text-sm leading-5 touch-manipulation cursor-pointer font-bold ${!pagination.hasPrevious ? 'opacity-50 cursor-not-allowed' : ''
                                    ...
                                    <button
                                        ...
                                        className={`relative flex button-group text-sm leading-5 touch-manipulation cursor-pointer font-bold ${!pagination.hasNext ? 'opacity-50 cursor-not-allowed' : ''
                                            } text-black h-10 px-6`}
                                        style={{ borderRadius: 0 }}
                                    >
                                        <div className='-inset-1 absolute z-0' style={{ borderRadius: '2.875rem' }}></div>
