...
export function PermissionsTable() {
  ...
  const [deleteData, setDeleteData] = useState<{
    ...
    data: { id?: string; name: string; module?: string };
  } | null>(null);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const draggedPermission = permissions.find((p) => p.id === active.id);
    ...
    >
      <div className='space-y-4'>
        ...
        <div className='grid gap-2 max-h-[calc(100vh-12rem)] overflow-y-auto pr-2'>
          {modules.map((moduleName) => {
            ...
            return (
              <SortableContext
                key={moduleName}
                items={modulePermissions.map((p) => p.id)}
              >
                <SortableTreeItem
                  id={moduleName}
        ...
        {(isDragging || permissions.some((p) => !p.module)) && (
          <DroppableArea>
            <SortableContext
              items={permissions.filter((p) => !p.module).map((p) => p.id)}
            >
        ...
        >
          {activeId ? (
            <div className='rounded-md border bg-background px-4 py-2 shadow-lg scale-105'>
              {permissions.find((p) => p.id === activeId)?.name}
            </div>
          ) : null}
        </DragOverlay>
      </div>
